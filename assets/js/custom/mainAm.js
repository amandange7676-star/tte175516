...
function loadAllRequiredContents(){
    ...
    components.forEach(component => {
        ...
        if(templatePath){
            loadContent(elementId, templatePath);
        }
...
if (isChecked) {
    ...
    if (!middleSectionsObject[selectedPage]) {
        ...
        if (!middleSectionsObject[selectedPage].includes(selectedId)) {
        ...
        success: function (response) {
            ...
            loadAllRequiredContents();
              if (CURRENT_MODE === 'design') {
                  $('.radio-holder.disabled input[type="checkbox"]:disabled').prop('disabled', false);
    $('.radio-holder.disabled').removeClass('disabled');
                  applyPageTypeView(); // restore correct section
...
$('input[name="pagesFor"]').on('change', function () {
    ...
    if (selectedType === "header" || selectedType === "footer") {
        ...
        $('#section-filter .selected-section').text("All");
